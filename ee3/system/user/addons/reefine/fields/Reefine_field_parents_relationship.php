<?php
class Reefine_field_parents_relationship extends Reefine_field {
	...
	function get_value_column($table='') {
		...
		else if ($this->child_field_name=='' || $this->child_field_name=='title')
	...
	function get_join_sql() {
		...
		"ON {$this->table_alias}.child_id = {$this->channel_data_alias}.entry_id " .
		...
		"ON {$this->table_alias_titles}.entry_id = {$this->table_alias}.parent_id " .
	...
	function get_where_clause($filter_group,$in_list=false,$value=false) {
		...
		if ($filter_group->join=='or' || $filter_group->join=='none') { // join="or" join="none" work like normal
			...
			if ($this->child_field_name!='' && $this->child_field_name!='title') {
				...
				"SELECT r.child_id AS entry_id FROM {$this->reefine->dbprefix}relationships r " .
